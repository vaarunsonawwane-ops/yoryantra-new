...
export default function ToolClient() {
 ...
 description="Review URL slugs for readability, casing, separators, length, optional keyword presence, and cleaner suggestions without treating a score as a ranking factor."
 >
...
function scoreSlug(issues: Issue[], status: SlugRow["status"]) {
 ...
 if (status === "long" && !issues.some((issue) => issue.title === "Long slug")) {
 score -= 10;
 }

 if (status === "short" && !issues.some((issue) => issue.title === "Very short slug")) {
 score -= 4;
 }
