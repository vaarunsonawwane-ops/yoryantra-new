...
export default function ToolClient() {
  ...
  return (
    <ToolShell
      ...
      description="Check JWT signing secret strength locally in your browser. Estimate practical entropy, detect weak/default secrets, repeated patterns, short keys, UUIDs, phrase-like values, and risky HMAC secret formats."
    >
...
function analyzeSecret(options: {
  ...
  const estimatedEntropy = estimatePracticalEntropy(normalizedSecret, charsetSize);
...
function estimatePracticalEntropy(value: string, charsetSize: number) {
  let entropy = value.length * Math.log2(Math.max(charsetSize, 1));
  const lowerValue = value.toLowerCase();

  if (/^[a-f0-9]+$/i.test(value) && value.length % 2 === 0) {
    entropy = Math.min(entropy, value.length * 4);
  }

  if (/^[A-Za-z0-9+/]+={0,2}$/.test(value) || /^[A-Za-z0-9_-]+$/.test(value)) {
    entropy = Math.min(entropy, value.length * 6);
  }
  ...
  if (/(secret|password|changeme|default|admin|qwerty|123456|your-256-bit-secret)/i.test(lowerValue)) {
    entropy *= 0.35;
  }
...
function detectFormat(value: string) {
  ...
  if (/^[a-f0-9]+$/i.test(value) && value.length >= 32 && value.length % 2 === 0) {
    return `hex-like (${value.length * 4}-bit shape)`;
  }
...
function buildIssues(options: {
  ...
  if (/\s/.test(options.secret)) {
    issues.push({
      severity: "warning",
      title: "Whitespace found",
      message: "Whitespace can be accidental in copied secrets. Confirm it is intentionally part of the key.",
    });
  }

  if (/^(sk-|pk_|ghp_|gho_|xox[baprs]-)/i.test(options.secret)) {
    issues.push({
      severity: "high",
      title: "Looks like a live service token",
      message: "This looks like an API token format, not a JWT signing secret. Rotate it if it was exposed.",
    });
  }
  ...
  if (issues.length === 0) {
    issues.push({
      ...
      message: "No obvious weakness was found by these local checks. This does not prove the secret was generated securely or stored safely.",
    });
  }
...
function formatOutput(result: Omit<SecretResult, "output">, mode: OutputMode) {
  ...
  return [
    ...
    "---------------------------",
    "This is a local estimate, not a full security audit.",
    `Secret preview: ${result.secretPreview}`,
    `Grade: ${result.grade}`,
    `Score: ${result.score}/100`,
    `Length: ${result.length}`,
